...
import { MissionsContent } from "@/components/ministries/MissionsContent";

type TinaQuery<T> = { query: string; variables: object; data: T };

type Props = {
  query: TinaQuery<MissionsPageQuery>;
};

export function ClientPage({ query }: Props) {
  const { data } = useTina(query);
  const page = data.missionsPage;
  ...
  return (
    <main>
      ...
      <MissionsContent page={page} lwml={page.lwml!} missionaries={page.missionaries!} />
    </main>
  );
}